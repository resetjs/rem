...
export interface ExFormProps extends ProFormProps {
    ...
    // 表单提交回到
    onSubmit?: (values: any, onHandle: (params: RequestOptions) => Promise<ResponseResult>) => Promise<ResponseResult>
    // 表单提交成功回调
...
const ExForm = (props: ExFormProps) => {
    ...
    const {
        ...
        read, renderTop, renderFooter, form: userForm, floatAction, selectedData,
        initialValues, mode, openid, group, formFields, onSubmit,
        onSubmitCallback, visible, onClose, handleCallback, modeProps, staticContext, layout, ...other
    } = props;

    const validGroups = group?.children.filter(item => !item.hasOwnProperty('show') || item.show) || [];
    ...
    const handleSubmit = () => {
        formProps.form?.validateFields()
            .then(transformSubmitValues).then(formatValues => onSubmit?.(formatValues, onHandle))
            .then(res => {
    ...
    const defaultSubmitButton = (
        <Space>
            ...
            <Button className={'rem-form-btn'}
                    onClick={() => history.length === 1 ? window.close() : history.goBack()}>返回</Button>
            }
            {
                (!group || group?.mode !== 'step' || current === validGroups.length - 1) &&
                <Button className={'rem-form-btn'} loading={isLoading} key='submit' type='primary'
    ...
    const defaultSubmitter = <Space>
        {mode === 'modal' && current === 0 &&
        <Button className={'rem-form-btn'} onClick={() => onClose?.(openid)}>取消</Button>}
        {
        ...
        switch (group.mode) {
            case 'sideMenu':
                dom = <Menu className={'rem-form-menu'}
                            ...
                            {...group.modeProps}
                >
                    {validGroups.map((item, position) => <Menu.Item key={position}>{item.label}</Menu.Item>)}
                </Menu>;
                break;
            case 'step':
                dom = (
                    <Steps current={current} className={'rem-form-steps'} {...group.modeProps}>
                        {validGroups.map(item => <Steps.Step title={item.label} key={item.key}/>)}
                    </Steps>
                );
                break;
            case 'radio':
                    ...
                    <div className={'rem-form-radio'}>
                        <Radio.Group
                            ...
                            {validGroups.map((item, index) =>
                                <Radio.Button key={`${index}`} value={`${index}`}>{item.label}</Radio.Button>)}
                        </Radio.Group>
                    </div>
                );
                break;
    ...
    const onValuesChange = async (changedValues: any, allValues: any) => {
        const element = formFields?.find(((item: any) => item.key === Object.keys(changedValues).toString()));
        if (element?.dependencies) {
            formProps?.form?.resetFields(element.dependencies);
        }
        formProps?.onValuesChange?.(transformSubmitValues(changedValues), transformSubmitValues(allValues));
    };
                ...
                <div className={isSideWay ? 'rem-form-content-menu' : 'rem-form-content'}>
                    ...
                    <ProForm {...formProps} onValuesChange={onValuesChange} name={openid || 'ExForm'}>
                        ...
                        <Form.Item
                            ...
                            {group?.mode === 'step' ? defaultSubmitter : defaultSubmitButton}</Form.Item>}
                    </ProForm>
                </div>
                {renderManual()}
            </div>
